...
class AdmReservas extends Component {
      constructor() {
      super();
      this.state={
          asignaciones:[],
          MostrarDialogoEliminarReserva: false,
          mostrarBoton:false,
      ...
      this.abrirAlertDatos=this.abrirAlertDatos.bind(this);
      this.abrirAlert=this.abrirAlert.bind(this);
      this.abrirAlertTabla=this.abrirAlertTabla.bind(this);
}

componentWillMount() {
  axios.get('http://127.0.0.1:8000/api/consultaGeneralReservas').then(res=>{
  ...
  eliminar = () =>  {
    axios.delete('http://localhost:8000/api/deleteReserva/'+this.state.id, {

    }).then(res=>{
      axios.get('http://localhost:8000/api/consultaGeneralReservas').then(res=>{
        const asignaciones = res.data;
        if(asignaciones.length > 0){
        this.setState({asignaciones, MostrarDialogoEliminarReserva:false});
        }
        else{
          this.setState({asignaciones:[], MostrarDialogoEliminarReserva:false});
          this.abrirAlertCerrar();
  ...
  handleClose1 = () => {
    this.setState({MostrarDialogoEliminarReserva: false});
  };
  ...
  obtenerReservaSeleccionadaEliminar(id, posicion) {
    ...
    this.setState({MostrarDialogoEliminarReserva: true});
  }
  ...
  setConsulta = evt =>{
    ...
    busquedaParametros = event => {
      ...
      axios.get("http://localhost:8000/api/consultaIdClienteReserva/" + busqueda).then(rest => {
        ...
        axios.get("http://localhost:8000/api/consultaIdHabitacionReserva/" + busqueda).then(rest => {
          const asignaciones = rest.data;
          if(asignaciones.length > 0){
          this.setState({asignaciones,mostrarText:false,buscar:''});
        }
        ...
        axios.get("http://localhost:8000/api/consultaFechaEntradaReserva/" + this.state.buscarfechaEntrada).then(rest => {
          const asignaciones = rest.data;
          if(asignaciones.length > 0){
          ...
          axios.get("http://localhost:8000/api/consultaHoraEntradaReserva/" + this.state.buscarhoraEntrada).then(rest => {
            const asignaciones = rest.data;
            if(asignaciones.length > 0){
...
render(){
  return (
    <div>
    <Principal/>
    <Dialog open={this.state.MostrarDialogoEliminarReserva} onClose={this.handleClose1} aria-labelledby="form-dialog-title">
            <DialogTitle id="form-dialog-title"><DeleteIcon/> Eliminar Reserva</DialogTitle>
            <DialogContent>
              <DialogContentText>
                NOTA: una vez eliminada la Reserva, se eliminará toda su información.
              </DialogContentText>
              <TextField
            ...
            <DialogActions>
              ...
              <Snackbar open={this.state.open} autoHideDuration={6000} onClose={this.cerrar}>
        ...
        <Grid container>
          ...
          <Grid>
          <h3 style={{color:"#bf360c" }}>Administración de Reservas</h3>
          </Grid>
      </Grid>
      ...
      { this.state.asignaciones.map((asignacion, key) =>
          <TableRow key={key}>
            ...
            <Button  onClick={() => {this.obtenerReservaSeleccionadaEliminar(asignacion.id, key)}} style={{color:'white', backgroundColor:'#bf360c'}} variant="contained" color='secondary' type="submit">
              <DeleteIcon/> Borrar
            </Button>
            </TableCell>
          </TableRow>  ) }
